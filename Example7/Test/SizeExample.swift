...
import CoreData
import Flow // https://github.com/tevelee/SwiftUI-Flow.git
import SwiftUI
...
struct SizeExample: View {
  ...
  @State private var colors222: [SkuEntity] = []
  
  @State private var isAppFirstLaunch: Bool = true
  
  @FetchRequest(
    entity: SkuEntity.entity(),
    sortDescriptors: []
  ) var colors: FetchedResults<SkuEntity>
  
  @FetchRequest(
    entity: SkuEntity.entity(),
    sortDescriptors: [NSSortDescriptor(keyPath: \SkuEntity.id, ascending: true)],
    animation: .default
  )
